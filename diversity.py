...
l_path = pickle.load(open(dir+'l_distance','rb'))
path = pickle.load(open(dir+'g_distance','rb'))
...
for m, v1 in path.items():
    v2 = l_path[m]
    l_d = average_c_dist(sorted(v2, key=v1.get)[:N])
    g_d = average_c_dist(sorted(v1, key=v2.get)[:N])
    w.write('%s,%s,%s\n'%(int(m), l_d, g_d))
